...
class _LawyerDetailsPageState extends State<LawyerDetailsPage> {
  ...
  builder: (context, constraints) {
    const double boxWidth = 100;      // نفس عرض البوكس الحالي
    const double spacing = 12;        // مسافة بين البوكسات

    // عدد العناصر اللي تقدر تدخل في السطر حسب عرض الشاشة
    final int perRow = (constraints.maxWidth / (boxWidth + spacing))
        .floor()
        .clamp(1, 3);

    // لو طلعوا 3 في سطر واحد خليه زي قبل، لو أقل خليه يتمدد شوي
    final double effectiveWidth = perRow == 3
        ? boxWidth
    ...
    return Wrap(
      ...
      children: [
        SizedBox(
          ...
          child: _roundedBox(Icons.work_outline, 'الخبرة', lawyer!['experience']),
        ),
        SizedBox(
          width: effectiveWidth,
          child: _roundedBox(Icons.school_outlined, 'التخصص الأكاديمي', lawyer!['academic']),
        ),
        SizedBox(
          width: effectiveWidth,
          child: _roundedBox(Icons.workspace_premium_outlined, 'الدرجة العلمية', lawyer!['degree']),
        ),
      ],
    );
  },
